...
void APZ_CityGenerator::BuildBlocksFromDelaunay(const TArray<FVector2D>& Points, const TArray<int32>& Triangles)
{
	...
	for (int32 s = 0; s < N; ++s)
	{
		...
		if (Corners.Num() < 3)
		{
			continue; // open/edge cell -- skip for now (border blocks)
		}
...
void APZ_CityGenerator::DrawDebugNetwork() const
{
	...
	for (const FPZ_Block& Block : Blocks)
	{
		...
		// Block outline = building footprint. Roads are the gaps between blocks.
		for (int32 i = 0; i < M; ++i)
		{
...
{
	// Circumcenter of triangle ABC (centre of the Voronoi vertex).
...
{
	// Simple inward inset: move each vertex toward the polygon centroid.
	// Good enough for greybox blocks; not a true straight-skeleton offset.
	...
	for (const FVector2D& P : Poly)
	{
		...
		{
			Dir /= Len;
			const float Move = FMath::Min(Inset, Len * 0.45f); // don't collapse past centre
			Out.Add(P + Dir * Move);
		}
		else
		{
...
TArray<FVector2D> APZ_CityGenerator::ClipToBounds(const TArray<FVector2D>& Poly, const FVector2D& HalfExtent)
{
	...
	auto ClipEdge = [](const TArray<FVector2D>& In, TFunctionRef<bool(const FVector2D&)> Inside, TFunctionRef<FVector2D(const FVector2D&, const FVector2D&)> Intersect)->TArray<FVector2D>
	{
		TArray<FVector2D> Out;
